...
/* ------------------------------------ */
if ( ! function_exists( 'boxcard_hex2rgb' ) ) {

	function boxcard_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'boxcard_enqueue_google_fonts' ) ) {

	function boxcard_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'boxcard_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'boxcard_dynamic_css' ) ) {

	function boxcard_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = boxcard_hex2rgb($color_1);
...
@media only screen and (max-width: 719px) {
	#nav-mobile.nav-container { border-bottom: 10px solid '.esc_attr( get_theme_mod('color-1') ).';  }
			...
			// card height
			if ( get_theme_mod('card-height','500') != '500' ) {
				$styles .= '.blog-card-inner { height: '.esc_attr( get_theme_mod('card-height') ).'px; }'."\n";
			}
			// card max-width
			if ( get_theme_mod('card-width','1200') != '1200' ) {
				$styles .= '
.blog-card,
.page-title .notebox,
.front-widgets { max-width: '.esc_attr( get_theme_mod('card-width') ).'px; }
				'."\n";
			}
			// content max-width
...
.page .entry-header,
.page .entry-footer,
.page .entry > *:not(.alignfull) { max-width: '.esc_attr( get_theme_mod('content-width') ).'px; }
				'."\n";
			}
			...
			wp_add_inline_style( 'boxcard-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'boxcard_dynamic_css' );
